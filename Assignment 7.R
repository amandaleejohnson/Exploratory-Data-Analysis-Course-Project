...
#Read in the data
        ...
        #0. Exploring the data, assessing whether there are negative values
        boxplot(combined$Emissions) 
        ...
        # from all sources for each of the years 1999, 2002, 2005, and 2008.
        library(plyr)                
        combined$total_emissions = ddply(combined, .(year), summarize, sum=ave(Emissions, FUN = sum))        
                #Using the base plotting system, make a plot showing the total PM2.5 emission 
                # from all sources for each of the years 1999, 2002, 2005, and 2008.
        us_aggtotals = aggregate(Emissions ~ year, combined, sum)

        barplot(us_aggtotals$Emissions, names.arg = us_aggtotals$year, 
        ...
        md$total_emissions = ddply(md, .(year), summarize, sum=ave(Emissions, FUN = sum))       
        md_aggtotals = aggregate(Emissions ~ year, md, sum)
        
        barplot(md_aggtotals$Emissions, names.arg = md_aggtotals$year, 
                main = "Total PM2.5 emissions (tons) in Baltimore, MD from 1999-2008", 
                xlab = "Year", 
                ylab = "Total PM2.5 emissions (tons)")
        ...
        #Plot with facets (facets = panels)
        qplot(year, Emissions, data = md, facets = .~type) + 
                geom_smooth(method = "lm", se = FALSE) +
                labs(title = "PM2.5 emissions (tons) in Baltimore, MD from 1999-2008", 
                        subtitle = "by Source Type") + 
                labs(x = expression("Year"), y = "PM2.5 emissions (tons)")
        ...
        qplot(year, Emissions, data = md_year_type_agg, facets = .~type) + 
                geom_smooth(method = "lm", se = FALSE) +
                labs(title = "PM2.5 emissions (tons) in Baltimore, MD from 1999-2008", 
                     subtitle = "by Source Type") + 
                labs(x = expression("Year"), y = "PM2.5 emissions (tons)")
        ...
        combust = combined[grepl("Fuel Comb", combined$EI.Sector, ignore.case = TRUE), ]
        us_coal_aggtotals = aggregate(Emissions ~ year, coal, sum)
        
        barplot(us_coal_aggtotals$Emissions, names.arg = us_coal_aggtotals$year, 
                main = "Total PM2.5 emissions in the US from 1999-2008 from coal combustion-related sources", 
                xlab = "Year", 
                ylab = "Total PM2.5 emissions (tons)")
        ...
        library(gmodels)
        CrossTable(vehicles$EI.Sector, vehicles$SCC.Level.Two)
        vehicles_md = md[grepl("vehicle", md$SCC.Level.Two, ignore.case = TRUE),]

        md_vehicles_aggtotals = aggregate(Emissions ~ year, vehicles_md, sum)
        
        yrange = range(md_vehicles_aggtotals$Emissions, na.rm=TRUE)        
        yrange
        barplot(md_vehicles_aggtotals$Emissions, names.arg = md_vehicles_aggtotals$year, 
                main = "Total PM2.5 emissions (tons) in Baltimore, MD from 1999-2008 from motor vehicle sources", 
                xlab = "Year", 
                ylab = "Total PM2.5 emissions (tons)", 
                ylim = c(0, max(md_vehicles_aggtotals$Emissions)))
        
        
        # 6. Compare emissions from motor vehicle sources in Baltimore City 
        # with emissions from motor vehicle sources in Los Angeles County, California (fips == "06037"). 
        # Which city has seen greater changes over time in motor vehicle emissions?
        la = merge(source_classification_code, subset(summarySCC_PM25, fips == "06037"))
        vehicles_la = la[grepl("vehicle", la$SCC.Level.Two, ignore.case = TRUE),]
        
        la_vehicles_aggtotals = aggregate(Emissions ~ year, vehicles_la, sum)
        
        la_vehicles_aggtotals$region = "Los Angeles County, CA"
        md_vehicles_aggtotals$region = "Baltimore City, MD"
        
        bothregions = rbind(la_vehicles_aggtotals, md_vehicles_aggtotals)
        
        yrange = range(la_vehicles_aggtotals$Emissions, md_vehicles_aggtotals$Emissions, na.rm=TRUE)        
        yrange
        
        qplot(year, Emissions, data = bothregions, facets = .~region) + 
                geom_smooth(method = "lm", se = FALSE) +
                labs(title = "PM2.5 emissions (tons) from motor vehicle sources in 1999-2008", 
                     subtitle = "by Region") + 
                labs(x = expression("Year"), y = "PM2.5 emissions (tons)") + 
                ylim(0, 8000)
